...
plot.totalvis <-
function(x, num_load = 5, rug = TRUE, ...) {
  ...
  plot(pred_df$x_vals, pred_df$avg_pred, type = "l", 
       xlab = paste0("PC", location), 
  		 main = paste0("PDP Plot of PC", location),
  ...
  legend_place <- ifelse(pred_df$avg_pred[1] <= 
  ...
  ## Add legend to plot
  legend(legend_place, legend = load_df$name, pch = load_df$symbol, 
